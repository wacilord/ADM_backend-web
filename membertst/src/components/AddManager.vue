...
export default {
  ...
  mounted() {
    if (sessionStorage.getItem("accountType") == 0) {
      if (!this.authUtil.isAuth(this, "addmanager")) {
        return;
      }
    }
    ...
    save() {
      ...
      if (
        ...
        if (this.queryParameters.accountID == this.queryParameters.password) {
          ...
          backendapiService.addmanager(this.queryParameters).then(res => {
